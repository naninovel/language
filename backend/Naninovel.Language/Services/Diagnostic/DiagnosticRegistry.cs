...
{
    private readonly Dictionary<string, List<DiagnosticRegistryItem>> uriToItems = [];
    private readonly List<Diagnostic> diags = [];

    public IReadOnlyList<DiagnosticRegistryItem> Get (string uri)
    {
        return uriToItems.TryGetValue(uri, out var items) ? items.ToArray() : [];
    }
    ...
    {
        return uriToItems.TryGetValue(uri, out var items) ? items : uriToItems[uri] = [];
    }
}
